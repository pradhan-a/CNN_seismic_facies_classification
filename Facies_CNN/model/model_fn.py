...
def model_fn(mode, inputs, params, reuse=False):
    ...
    with tf.variable_scope('model', reuse=reuse):
        ...
        if params.model_version=='baseline':
            ...
            logits = build_dilatedconv_model(is_training, inputs, params)
        predictions = tf.argmax(logits, -1)
        assert predictions.get_shape().as_list() == [None, params.output_size_z, params.input_size_x, params.input_size_y]
...
def build_dilatedconv_model(is_training, inputs, params):
    ...
    assert seis.get_shape().as_list() == [None, params.input_size_z, params.input_size_x, params.input_size_y, 1]
    ...
    # Define the number of channels of each convolution
    # For each block, we do: 3x3 conv -> batch norm -> relu -> 2x2 maxpool
    # Upsample to make time and depth same dimensions
    out=tf.layers.conv3d_transpose(out, 8, (35,1,1), 1, padding='valid')
    if params.use_batch_norm:
                out = tf.layers.batch_normalization(out, momentum=bn_momentum, training=is_training)
    out = tf.nn.relu(out)

    out=tf.layers.conv3d_transpose(out, 16, (34,1,1), (2,1,1), padding='valid')
    if params.use_batch_norm:
        out = tf.layers.batch_normalization(out, momentum=bn_momentum, training=is_training)
    out = tf.nn.relu(out)


    channels_encoder = [num_channels for i in np.arange(num_layers)]
    channels_encoder[-1]=int(num_channels/2)
    dilate_factors=[2**(i) for i in np.arange(num_layers-1)]
    #dilate_factors.insert(0,1)
    dilate_factors.append(1)


    for i, c in enumerate(channels_encoder):
        with tf.variable_scope('encoder_block{}'.format(i+1)):
            d=dilate_factors[i]
            out = tf.layers.conv3d(out, c, 3, 1, padding='same',dilation_rate=(d,d,d))
            if params.use_batch_norm:
                out = tf.layers.batch_normalization(out, momentum=bn_momentum, training=is_training)
            out = tf.nn.relu(out)
