...
def build_dilatedconv_model(is_training, inputs, params):
    ...
    bn_momentum = params.bn_momentum
    channels_encoder = [num_channels*(2**i) for i in np.arange(num_layers)]
    regularizer = tf.contrib.layers.l2_regularizer(scale=params.l2_reg)

    for i, c in enumerate(channels_encoder):
        with tf.variable_scope('encoder_block{}'.format(i+1)):
            out = tf.layers.conv2d(out, c, 3, 1, padding='same',kernel_regularizer=regularizer)
            if params.use_batch_norm:
                out = tf.layers.batch_normalization(out, momentum=bn_momentum, training=is_training)
            out = tf.nn.relu(out)
            if i==num_layers-1:
    ...
    for i, c in enumerate(channels_dilate):
        with tf.variable_scope('dilate_block{}'.format(i+1)):
            d=dilate_factors[i]
            out = tf.layers.conv2d(out, c, 3, 1, padding='same',dilation_rate=(d,d),kernel_regularizer=regularizer)
            if params.use_batch_norm:
                out = tf.layers.batch_normalization(out, momentum=bn_momentum, training=is_training)
            out = tf.nn.relu(out)

    out = tf.layers.conv2d(out, 16, 1, 1, padding='same',kernel_regularizer=regularizer)
    if params.use_batch_norm:
        out = tf.layers.batch_normalization(out, momentum=bn_momentum, training=is_training)
    out = tf.nn.relu(out)

    out = tf.layers.conv2d(out, params.num_labels, 1, 1, padding='same',kernel_regularizer=regularizer)
    if params.use_batch_norm:
        out = tf.layers.batch_normalization(out, momentum=bn_momentum, training=is_training)
